...
echo "$($PSQL "select distinct r.name from (select name, round, year from games inner join teams on games.winner_id=teams.team_id union select name, round, year from games inner join teams on games.opponent_id=teams.team_id) as r where r.round='Eighth-Final' and r.year=2014 order by r.name;")"
